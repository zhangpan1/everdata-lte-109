...
//public class SdtpClientLteIoHandler2 implements IoHandler {
...
//			List<XdrSingleSource> datas) {
//		SEND_DATA_INFO.incrementAndGet();
//		NotifyXDRDataReq req =  createNotifyXDRDataReq(datas);
//		session.write(req);
...
//	}
//
//	private NotifyXDRDataReq createNotifyXDRDataReq(List<XdrSingleSource> datas) {
//		int totalLength = 0;
//		// 注意头长度变化和内容一定要对的上
...
//				SEQ.incrementAndGet(), (short) datas.size());
//		req.setHeader(header);
//		return req;
//	}
//
...
//		SEND_DATA_INFO.incrementAndGet();
//		NotifyXDRDataReq req = createCompXdrDataSendReq(datas);
//		session.write(req);
//		XDR_2_SDTP_BYTES.addAndGet(req.getHeader().getTotalLength());
//		XDR_2_SDTP_PACKAGE.incrementAndGet();
//
...
//	}
//
//	private NotifyXDRDataReq createCompXdrDataSendReq(
//			List<XdrCompoundSource> datas) {
//		// 注意头长度变化和内容一定要对的上
//		int totalLength = 0;
//		totalLength += datas.size();
...
//		SdtpHeader header = new SdtpHeader(SDTP_HEADER_LENGTH
//				+ req.getBodyLength(), NOTIFY_XDR_DATA_REQ,
//				SEQ.incrementAndGet(), (byte) datas.size());
//		req.setHeader(header);
//		return req;
//	}
//
...
//		SEND_DATA_INFO.incrementAndGet();
//		NotifyXDRDataReq req =createCompXdrDataSendReq(data);
//		session.write(req);
//		XDR_2_SDTP_BYTES.addAndGet(req.getHeader().getTotalLength());
//		XDR_2_SDTP_PACKAGE.incrementAndGet();
//
...
//	}
//
//	private NotifyXDRDataReq createCompXdrDataSendReq(XdrCompoundSource data) {
...
//									.take();
//							ArrayList<XdrSingleSource> list = new ArrayList<>();
//							for (int i = 0; i < data.length; i++) {
//								list.add(data[i]);
//							}
//							doXDRDataSendReq(session, list);
//						} catch (Exception e) {
//							logger.error(Arrays.toString(data) + "", e);
//							session.close(true);
//							return;
//						}
//					}
...
//									.take();
//							List<XdrCompoundSource> list = new ArrayList<>();
//							for (int i = 0; i < data.length; i++) {
//								list.add(data[i]);
//							}
//							doCompXdrDataSendReq(session, list);
//						} catch (Exception e) {
//							logger.error(Arrays.toString(data) + "", e);
//							session.close(true);
//							return;
