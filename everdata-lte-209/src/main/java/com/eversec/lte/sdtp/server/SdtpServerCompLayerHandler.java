...
public class SdtpServerCompLayerHandler implements IoHandler {
	...
	public void messageReceived(IoSession session, Object message)
		...
		case NOTIFY_XDR_DATA_REQ://XDR数据
			...
			break;
		case XDR_RAW_DATA_SEND_REQ://原始码流
	...
	protected NotifyXDRDataResp processNotifyXDRDataResp(SdtpHeader header,
		...
		if( SdtpConfig.IS_PROCESS_XDR){
			if (SdtpConfig.IS_BACKFILL) {//判断是否是回填
				...
				if (SdtpConfig.IS_OUTPUT_ORIGINAL_XDR_2_KAFKA) {//放到队列中，等待放入kafka中
				...
				if(SdtpConfig.IS_OUTPUT_ORIGINAL_XDR_2_EMPTY){//什么都不做
	...
	protected XDRRawDataSendResp processXDRRawDataSendResp(SdtpHeader header,
		...
		if( SdtpConfig.IS_PROCESS_RAW){
			...
			if (SdtpConfig.IS_OUTPUT_ORIGINAL_RAW_2_KAFKA) {//放kafka
